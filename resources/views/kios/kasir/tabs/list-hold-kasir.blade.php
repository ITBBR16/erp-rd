<div class="hidden p-4" id="list-hold" role="tabpanel" aria-labelledby="list-hold-tab">
            ...
            <tbody>
                @foreach ($dataHold as $data)
                    <tr class="bg-white border-b hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-600">
                        <th class="px-6 py-2">
                            {{ $data->customer->first_name }} - {{ $data->customer_id }} - {{ $data->created_at->format('Ymd') }}{{ $data->id }}
                        </th>
                        <td class="px-6 py-2">
                        ...
                        <td class="px-6 py-2">
                            @php
                                $totalNominal = $data->ongkir + $data->tax + $data->total_harga - $data->discount;
                            @endphp
                            Rp. {{ number_format($totalNominal, 0, ',', '.') }}
                        </td>
                        <td class="px-6 py-2">
                             <span class="bg-yellow-100 text-yellow-700 text-xs font-bold me-2 px-2.5 py-0.5 rounded dark:bg-yellow-800 dark:text-yellow-300">{{ $data->status }}</span>
                        </td>
                        <td class="px-6 py-2">
                            <button id="dropdownListHoldKasir{{ $data->id }}" data-dropdown-toggle="ddHoldKasir{{ $data->id }}" data-dropdown-placement="bottom" class="text-gray-500 border border-gray-300 font-bold rounded-lg text-sm p-2 w-32 text-start inline-flex items-center dark:text-gray-300 dark:border-gray-300" type="button">Atur <svg class="w-2.5 h-2.5 ms-16" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
                    ...
                    <!-- Dropdown menu -->
                    <div id="ddHoldKasir{{ $data->id }}" class="z-10 hidden bg-white rounded-lg shadow w-40 dark:bg-gray-700">
                        <ul class="h-auto py-2 text-gray-700 dark:text-gray-200" aria-labelledby="dropdownListHoldKasir{{ $data->id }}">
                            <li>
                            ...
                            <li>
                                <a href="{{ route('kasir.edit', encrypt($data->id)) }}" class="flex w-full items-center px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 text-gray-400 hover:text-gray-800 dark:hover:text-gray-300">
