            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="px-6 py-6 lg:px-8">
                    ...
                    @foreach ($item->orderLists as $key => $ol)
                        <div class="grid grid-cols-4 gap-4">
                            <div class="relative col-span-2 z-0 w-full mb-4 group">
                                <input type="text" id="nama-produk-{{ $key }}" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" value="{{ $ol->paket->paket_penjualan }}" disabled>
                                <label for="nama-produk-{{ $key }}" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Nama Produk</label>
                            </div>
                            <div class="relative z-0 w-full mb-4 group">
                                <input type="text" id="quantity-{{ $key }}" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" value="{{ $ol->quantity }}" disabled>
                    ...
                    <div class="relative z-0 w-full my-3 group">
                        <a href="{{ $item->link_drive }}" id="link_drive{{ $item->id }}" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none hover:text-blue-600 dark:hover:text-blue-500 dark:text-white dark:border-gray-600 peer" target="_blank" rel="noopener noreferrer">
                            {{ $item->link_drive }}
                        </a>
