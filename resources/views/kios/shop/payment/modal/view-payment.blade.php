@foreach ($payment as $viewPayment)
    @if ($viewPayment->status == 'Unpaid' || $viewPayment->status == 'Waiting For Payment')
        <div id="view-belum-terbayar-{{ $viewPayment->id }}" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative w-full max-w-xl max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    <div class="flex items-center justify-between p-5 border-b rounded-t dark:border-gray-600">
                        <h3 class="text-xl font-medium text-gray-900 dark:text-white">Detail Payment Order {{ ($viewPayment->order_type == 'Baru') ? 'N.' . $viewPayment->order_id : 'S.' . $viewPayment->order_id }} / <span class="text-sm bg-orange-400 rounded-md px-2 py-0 text-white">{{ $viewPayment->status }}</span></h3>
                        <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="view-belum-terbayar-{{ $viewPayment->id }}">
                    ...
                    <div class="px-6 py-6 lg:px-8">
                        ...
                        <div class="grid md:grid-cols-2 md:gap-6">
                            <div class="relative z-0 w-full mb-6 group">
                                {{-- <input type="text" id="media_transaksi{{ $viewPayment->id }}" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" value="{{ ($viewPayment->order_type == 'Baru') ? $viewPayment->metodepembayaran->akunBank->nama : $viewPayment->metodepembayaransecond->akunBank->nama }}" disabled> --}}
                            ...
                            <div class="relative z-0 w-full mb-6 group">
                                {{-- <input type="text" id="no_rek{{ $viewPayment->id }}" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" value="{{ ($viewPayment->order_type == 'Baru') ? $viewPayment->metodepembayaran->no_rek : $viewPayment->metodepembayaransecond->no_rek }}" disabled> --}}
                        ...
                        <div class="relative z-0 w-full mb-6 group">
                            {{-- <input type="text" id="nama_akun{{ $viewPayment->id }}" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" value="{{ ($viewPayment->order_type == 'Baru') ? $viewPayment->metodepembayaran->nama_akun : $viewPayment->metodepembayaransecond->nama_akun }}" disabled> --}}
