@foreach ($payment as $item)
    <div id="edit-pembayaran{{ $item->id }}" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative w-full max-w-xl max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                <div class="flex items-center justify-between p-5 border-b rounded-t dark:border-gray-600">
                    <h3 class="text-xl font-medium text-gray-900 dark:text-white">Edit Payment Order K.{{ $item->order->id }}</h3>
                    <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="edit-pembayaran{{ $item->id }}">
                ...
                <div class="px-6 py-6 lg:px-8">
                    <form action="{{ route('pembayaran.update', $item->id) }}" method="POST" autocomplete="off">
                        @csrf
                        @method('PUT')
                        <div class="grid md:grid-cols-2 md:gap-6 pt">
                            ...
                            @enderror
                            <div class="relative z-0 w-full mb-6 group">
                                <input type="text" name="invoice" id="invoice" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" value="{{ $item->order->invoice }}" disabled>
                        ...
                        <div id="new-payment-metode-edit" style="display: none">
                            ...
                            <div class="relative z-0 w-full mb-6 group">
                                <input type="text" name="nama_akun" id="nama_akun" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer @error('nama_akun') border-red-600 dark:border-red-500 @enderror" placeholder="" value="{{ old('nama_akun') }}">
                        ...
                        <div class="grid grid-cols-3 md:gap-6">
                            ...
                            <div class="relative z-0 w-full mb-6 group">
                                <input type="text" name="ongkir" id="ongkir" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer @error('ongkir') border-red-600 dark:border-red-500 @enderror" placeholder="" value="Rp. {{ number_format($item->ongkir, 0, ',', '.') }}">
