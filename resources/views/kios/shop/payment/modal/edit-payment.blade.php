                ...
                <div class="px-6 py-6 lg:px-8">
                    <form action="{{ route('pembayaran.update', $editPayment->id) }}" method="POST" autocomplete="off">
                        ...
                        <div class="grid md:grid-cols-2 md:gap-6 pt">
                            ...
                            @error('tanggal_req{{ $editPayment->id }}')
                                <p class="mt-2 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="flex">
                        ...
                        <div id="new-payment-metode-edit-{{ $editPayment->id }}">
                            <div class="grid md:grid-cols-2 md:gap-6">
                                ...
                                <div class="relative z-0 w-full mb-6 group">
                                    <input type="text" name="no_rek" id="no-rek-{{ $editPayment->id }}" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer @error('no-rek-{{ $editPayment->id }}') border-red-600 dark:border-red-500 @enderror" placeholder="" value="{{ old('no-rek', ($editPayment->order_type == 'Baru' && $editPayment->metodepembayaran) ? $editPayment->metodepembayaran->no_rek : (($editPayment->order_type == 'Bekas' && $editPayment->metodepembayaransecond) ? $editPayment->metodepembayaransecond->no_rek : '')) }}" readonly>
