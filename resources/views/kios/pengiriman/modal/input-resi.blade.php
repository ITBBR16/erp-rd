                    ...
                    <form action="{{ route('pengiriman.update', $item->id) }}" method="POST" autocomplete="off">
                        ...
                        <div class="grid md:grid-cols-2 md:gap-6 pt">
                            <div class="relative z-0 w-full mb-6 group">
                                ...
                                <input type="text" id="order_id{{ $item->id }}" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" value="{{ ($item->status_order == 'Baru' && $item->order ? 'K.' . $item->order->id : ($item->status_order == 'Bekas' && $item->ordersecond ? 'S.' . $item->ordersecond->id : '')) }}" required>
