                ...
                <tbody>
                    ...
                    @foreach ($groupedItems as $items)
                        ...
                        @if ($qc->qualityControll?->status_qc != 'Tervalidasi')
                            ...
                            <!-- Dropdown menu -->
                            <div id="dropdownQCL{{ $qc->id }}{{ $qc->produk_gudang_id }}" class="z-10 hidden bg-white rounded-lg shadow w-40 dark:bg-gray-700">
                                <ul class="h-auto py-2 text-gray-700 dark:text-gray-200" aria-labelledby="dropdownListQC{{ $qc->id }}{{ $qc->produk_gudang_id }}">
                                    <li>
                                        <button type="button" data-modal-target="detail-qc-{{ $qc->id }}" data-modal-toggle="detail-qc-{{ $qc->id }}" class="flex w-full items-center px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 text-gray-400 hover:text-gray-800 dark:hover:text-gray-300">
                                    ...
                                    @if ($qc->qualityControll?->status_qc == 'Menunggu QC')
                                        <li>
                                            <a href="{{ route('qcFisik', ['idBelanja' => $qc->gudang_belanja_id, 'idProduk' => $qc->gudang_produk_id]) }}" class="flex w-full items-center px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 text-gray-400 hover:text-gray-800 dark:hover:text-gray-300">
                                                <i class="material-symbols-outlined text-xl mr-3">build</i>
                                                <span class="whitespace-nowrap">QC Fisik</span>
                                            </a>
                                        </li>
                                    @endif
                                    @if ($qc->qualityControll?->checked_quantity != '')
                                        <li>
                                            <a href="{{ route('qcFungsional', ['idBelanja' => $qc->gudang_belanja_id, 'idProduk' => $qc->gudang_produk_id]) }}" class="flex w-full items-center px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 text-gray-400 hover:text-gray-800 dark:hover:text-gray-300">
                                                <i class="material-symbols-outlined text-xl mr-3">manufacturing</i>
                                                <span class="whitespace-nowrap">QC Fungsional</span>
                                            </a>
                                        </li>
                                    @endif
                                </ul>
