    ...
    <nav class="flex my-6">
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            <li class="flex items-center">
                <a href="{{ route('gudang-validasi.index') }}" class="flex items-center text-sm font-medium text-gray-700 hover:text-blue-600 dark:text-gray-400 dark:hover:text-white">
                    <span class="material-symbols-outlined w-5 h-5">rule</span>
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">Validasi Sparepart N.{{ $idBelanja }}</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <form action="{{ route('gudang-validasi.store') }}" method="POST" autocomplete="off">
        ...
        <div class="relative my-4">
            <div class="flex items-center justify-between">
                <div class="flex text-xl justify-start">
                    <span class="text-gray-700 font-semibold dark:text-gray-300">List Validasi</span>
                </div>
        ...
        <div class="relative">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    @foreach ($dataCekValidasi as $item)
                        <tr class="bg-white border-b border-gray-200 hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-600">
                            <td class="px-6 py-2">
                                <input type="hidden" name="idItemId[]" value="{{ $item->id }}">
                                <input type="hidden" name="qc_id[]" value="{{ $item->qualityControll->id }}">
                                @php
                                    $sku = $item->produkSparepart->produkType->code . "." . $item->produkSparepart->partModel->code . "." . 
                                            ...
                                            $item->produkSparepart->partSubBagian->code . "." . $item->produkSparepart->produk_part_sifat_id . "." .
                                            $item->produkSparepart->id;
                                @endphp
                                {{ $sku }}
                            </td>
                            <td class="px-6 py-2">
                            ...
                            <td class="px-6 py-2">
                                {{ $item->qualityControll->checked_quantity }}
                            </td>
                            <td class="px-6 py-2">
                                {{ $item->qualityControll->checked_fisik }}
                            </td>
                            <td class="px-6 py-2">
                                {{ $item->qualityControll->checked_fungsional ?? "-" }}
                            </td>
                            <td class="px-6 py-2">
                                <select name="cek_validasi[]" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
                                    <option value="" hidden>Status Validasi</option>
                                    <option value="Pass">Pass</option>
                                    <option value="Komplain">Komplain</option>
                                </select>
                            </td>
                            <td class="px-6 py-2">
                                <textarea name="keterangan[]" rows="2" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Keterangan . . ." ></textarea>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
