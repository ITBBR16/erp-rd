...
@section('container')
    ...
    <div class="relative">
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                @php
                    $filteredComplaints = $listKomplain->filter(function ($item) {
                        return optional($item->qualityControll)->status_validasi === 'Komplain';
                    });

                    $groupedItems = $filteredComplaints->groupBy(function ($item) {
                ...
                @foreach ($groupedItems as $items)
                    @php
                        $qc = $items->first();
                        $quantityCount = $items->filter(function ($item) {
                            return optional($item->qualityControll)->status_validasi === 'Komplain';
                        })->count();
                    @endphp
