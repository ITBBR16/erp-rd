@foreach ($dataCase as $case)
    @if ($case->jenisStatus->jenis_status == 'Proses Pengerjaan')
        <div id="detail-pengerjaan-{{ $case->id }}" tabindex="-1" class="modal fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative w-full max-w-7xl max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="flex items-center justify-between p-5 border-b rounded-t dark:border-gray-600">
                        ...
                        <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="detail-pengerjaan-{{ $case->id }}">
                    ...
                    <div class="px-2 py-2 lg:px-8 lg:py-6 bg-gray-50 dark:bg-gray-600">
                        <div class="grid grid-cols-2 gap-6">
                            <div class="p-4 rounded-lg bg-white border shadow-md dark:bg-gray-700 dark:border-gray-600">
                                ...
                                <h3 class="text-lg font-semibold mb-4 pt-2 border-t dark:text-white">Detail Pesan Estimasi</h3>
                                <div>
                                    <h3 class="text-sm font-semibold mb-1">Note Estimasi</h3>
                                    <p class="text-gray-500 text-base">
                                        @foreach ($case->timestampStatus->where('jenis_status_id', 5) as $timeStamp)
                            ...
                            <div class="p-4 rounded-lg bg-white border shadow-md dark:bg-gray-700 dark:border-gray-600">
                                ...
                                <div class="relative overflow-y-auto">
                                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                        ...
                                        <tbody>
                                            @foreach ($case->estimasi->estimasiPart as $part)
                                                @if ($part->active == 'Active')    
                                                    <tr class="bg-white dark:bg-gray-800">
                                                        <th class="px-6 py-4 font-medium text-gray-900 dark:text-white">
                                                            {{ $part->jenisTransaksi->code_jt }}
                                                        </th>
                                                        <td class="px-6 py-4 items-center">
                                                            {{ $part->sparepartGudang->produkSparepart->produkJenis->jenis_produk }}
                                                        </td>
                                                        <td class="px-6 py-4 items-center">
                                                            {{ $part->sparepartGudang->produkSparepart->nama_internal }}
                                                        </td>
                                                    </tr>
                                                @endif
                                            @endforeach
