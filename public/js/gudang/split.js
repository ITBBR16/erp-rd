$(document).ready(function () {
    ...
    $(document).on('change', '#nama-sparepart', function () {
        ...
        .then(data => {
            ...
            data.listIdItem.forEach(idItem => {
                const nameII = 'N.' + idItem.gudang_belanja_id + '.' + idItem.gudang_belanja.gudang_supplier_id + '.' + idItem.id;
                const option = $('<option>')
                    .val(idItem.id)
                    .text(nameII)
    ...
    $(document).on('change', '.qty-split, .nominal-split, .qty-split-optional, .nominal-split-optional', function () {
        updateBoxSplit();
    });
    ...
    function updateBoxSplit() {
        ...
        $('#container-split-part .form-list-split').each(function () {
            ...
            let nominal = parseFloat($(this).find('.nominal-split').val().replace(/\./g, '')) || 0
            let hasilNominal = qty * nominal;
            totalNominal += hasilNominal;
        });
        
        $('#container-optional-split-part .form-list-optional-split').each(function () {
            let qtyOptional = parseFloat($(this).find('.qty-split-optional').val().replace(/\./g, '')) || 0
            let nominalOptional = parseFloat($(this).find('.nominal-split-optional').val().replace(/\./g, '')) || 0
