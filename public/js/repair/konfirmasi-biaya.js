...
$(document).ready(function () {
        ...
        itemForm += `
                ...
                <td class="px-2 py-4">
                    <div class="flex">
                        ...
                        <input type="text" name="harga_customer[]" id="harga-customer-${countForm}" class="format-angka-estimasi nominal_customer rounded-none rounded-r-lg bg-gray-50 border text-gray-900 focus:ring-blue-500 focus:border-blue-500 block flex-1 min-w-0 w-full text-sm border-gray-300 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="0" required>
                    </div>
                </td>
        ...
        let itemFG = `
            <tr id="konfirmasi-data-gudang-${countForm}" class="bg-white dark:bg-gray-800">
                ...
                <td class="px-2 py-4">
                    <div class="relative z-0 w-full group flex items-center">
                        <span class="absolute start-0 font-bold text-gray-500 dark:text-gray-400">RP</span>
                        <input name="harga_repair_baru[]" id="harga-repair-${countForm}" type="text" class="format-angka-estimasi block py-2.5 ps-6 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" readonly>
                    </div>
                </td>
                <td class="px-2 py-4">
                    ...
                    <div class="relative z-0 w-full group flex items-center">
                        <span class="absolute start-0 font-bold text-gray-500 dark:text-gray-400">RP</span>
                        <input name="harga_gudang_baru[]" id="harga-gudang-${countForm}" type="text" class="format-angka-estimasi block py-2.5 ps-6 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" readonly>
                    </div>
                </td>
            </tr>
        `;
    ...
    $(document).on("input", ".format-angka-estimasi", function () {
        ...
        totalBiayaUbahEstimasi();
    });

    $(document).on("change", ".estimasi-jt", function () {
        let formId = $(this).data("id");
        var jenisTransaksi = $(
            "#estimasi-jt-" + formId + " option:selected"
        ).text();
    ...
    $(document).on("change", ".konfirmasi-jp", function () {
        ...
        var jenisTransaksi = $(
            "#estimasi-jt-" + jpId + " option:selected"
        ).text();
    ...
    $(document).on("change", ".konfirmasi-part", function () {
        ...
        var sku = $(this).val();
        var jenisTransaksi = $(
            "#estimasi-jt-" + partId + " option:selected"
        ).text();
        console.log(sku);
        if (jenisTransaksi == "P.Baru" || jenisTransaksi == "P.Bekas") {
    ...
    $(document).on("click", ".deactive-form-estimasi", function () {
        ...
        $(this)
            .removeClass("deactive-form-estimasi")
            .addClass("activate-button");
    ...
    $(document).on("click", ".activate-button", function () {
        ...
        $(this)
            .removeClass("activate-button")
            .addClass("deactive-form-estimasi");
    ...
    function totalBiayaUbahEstimasi() {
        let totalBiaya = 0;
        $("#container-estimasi-active .format-angka-estimasi").each(
            function () {
                const value = $(this).val();
                totalBiaya += parseRupiahToNumber(value);
            }
        );
        $("#total-nominal-ubah-estimasi-biaya").text(
            "Rp. " + formatAngka(totalBiaya)
        );
    }
});
